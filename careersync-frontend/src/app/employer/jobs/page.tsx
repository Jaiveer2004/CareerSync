...
import { RecruiterJobCardSkeleton } from "@/components/employer/RecruiterJobCardSkeleton";
        ...
        <div className="container mx-auto py-8 px-4 space-y-8">
          ...
          {isLoading ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 6 }).map((_, i) => <RecruiterJobCardSkeleton key={i} />)}
            </div>
          ) : services.length > 0 ? (
                ...
                {filteredServices.map((service) => (
                  <div key={service._id} className="bg-white rounded-2xl p-6 border border-slate-200 hover:border-slate-300 transition-all duration-200 shadow-lg hover:shadow-xl group">
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100">
                      <Link href={`/employer/jobs/${service._id}/applicants`} className="block w-full">
                        <Button className="w-full bg-indigo-50 hover:bg-indigo-600 text-indigo-700 hover:text-white rounded-lg py-2 font-bold transition-all text-xs border border-indigo-100">
                          View Applicants
                        </Button>
                      </Link>
                    </div>
