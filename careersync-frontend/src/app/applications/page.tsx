...
import { BookingCardSkeleton } from "@/components/applications/ApplicationCardSkeleton";
import { DashboardLayout } from "@/components/layouts/DashboardLayout";
import { EnhancedBookingCard } from "@/components/applications/EnhancedApplicationCard";
import { BookingFilters } from "@/components/applications/ApplicationFilters";
import { BookingSummary } from "@/components/applications/ApplicationSummary";
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16">
              <div className="bg-white rounded-2xl p-12 border border-slate-200 shadow-sm max-w-md mx-auto">
                ...
                <button 
                  onClick={() => window.location.href = user?.role === 'customer' ? '/jobs' : '/employer/jobs'}
                  className="bg-[#1e40af] hover:bg-[#1e3a8a] text-white px-6 py-3 rounded-xl font-medium transition-all duration-200"
                >
                  {user?.role === 'customer' ? 'Browse Jobs' : 'Manage Jobs'}
                </button>
              </div>
            </div>
          ) : (
